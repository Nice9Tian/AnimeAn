...
- **左列**：上为 `child_paint_view`（子画板/texture view，独立场景模型），下为工具栏（Tools）。子画板是**无限画布**：整个视口都是纸面（淡灰框标出名义页面作参照），平移不受页面边界限制（滚轮缩放 + 中键平移，无滚动条），图案画在哪都行 — 反正映射按中心线缩放。
...
- 从起笔点（或中途按下 Alt 的那一点）算起，鼠标位移一旦超过阈值（默认 **5 px**），按主方向锁定轴向 — 横向位移大则水平、纵向大则垂直；锁定前累积的微小抖动会被回溯拉直，不留起笔小钩。
...
- 阈值可通过 C++ 侧 `PaintOpenGLWidget::setAxisSnapThreshold()` 调整（暂未暴露到选项面板）。
...
工具栏新增 ExtraTool：`H Center Line`、`V Center Line`、`Mapping Area`、`Auto Mapping`、`Auto Mapping 2`。

**两种映射算法**（共享同一套端点锚定参数化，结果都写入 mapped layer、互相替换）：

- `Auto Mapping`（脊柱旋转）：V 方向位移在 H 脊柱的局部坐标系里随切向旋转 — 弯曲处图案截面跟着转（横幅沿路径效果），远端不衰减。
- `Auto Mapping 2`（Coons 插值）：按象限做纯 Coons 补丁插值（公式坍缩为 `H(s)+V(t)−O`）— 位移只平移不旋转，弯曲处图案是"平移副本"，绝无旋转引入的折叠。直线引导下两者完全一致。哪个更符合直觉可以用 Refer Rect 网格对比后选用。

**Refer Rect（映射网格检查）**：选中任一 Auto Mapping 工具后，选项面板有 `Refer Rect` 开关。开启后两个画板同时叠加橙色 3×3 锚点网格 — 锚点即十字线定义的 9 个点（交点、4 个线端点、4 个象限角点），child 侧显示参考框本身，main 侧显示它经**当前算法**映射后的像 — 网格哪里歪了，映射问题就在哪里。网格随引导线修改、撤销、重新映射自动刷新。
