...
```
Φ(p) = p − hv_child(ℓ_h, ℓ_v) + hv_main( μ_h(ℓ_h), μ_v(ℓ_v) )
                └──── 残差项 ────┘   └──── 重建项 ────┘
```
...
flowchart TD
    ...
    B --> C{{"阻尼牛顿反解<br/>_Frame.solve"}}
    C --> D["弧长坐标 ℓ_h, ℓ_v"]
    D --> E["child 侧重建<br/>rebuilt = hv_child(ℓ)"]
    D --> F["逐侧标定 μ<br/>ℓ 取正用 pos 尺度<br/>ℓ 取负用 neg 尺度"]
    F --> G["main 侧重建<br/>image = hv_main(μ(ℓ))"]
    E --> H["残差 r = p − rebuilt"]
    G --> I["Φ(p) = image + r"]
    H --> I
    I --> J["映射点位置"]
...
* **重建项** `hv_main(μ(ℓ))` —— 承担全部形变，是"预测"的主体。
* **残差项** `p − hv_child(ℓ)` —— 反解成功时**恒为零**；反解失败时它兜住误差，让映射保持连续、且不比旧实现更差。
...
3. **无下坡路 = 合法终止**，不是 bug。此时 `break` 保留历史最优，交给残差项兜底。
...
四个出口中，只有**出口①**代表 `ℓ` 精确；其余三个都返回"见过的最优"，误差由残差项承接（§7）。
...
## 7. 第五步：重建 + 残差项，以及恒等性定理

```python
def map_point(point):
    l_h, l_v = coords(point)
    rebuilt = child.hv(l_h, l_v)
    image   = main.hv(μ_h(l_h), μ_v(l_v))
    return (image[0] + point[0] - rebuilt[0],
            image[1] + point[1] - rebuilt[1])
```

**恒等性定理**：若 child 与 main 画了完全相同的导引线，则 `Φ(p) = p`，**对任意导引线形状成立，且不要求牛顿收敛**。

证明一行：两板相同 ⇒ `μ = id` 且 `hv_main ≡ hv_child` ⇒ `image = rebuilt` ⇒ `Φ(p) = p + (image − rebuilt) = p`。∎

注意这个证明**没有用到** `hv_child(ℓ) = p`。也就是说残差项让恒等性对反解失败的点同样成立。实测印证（§9 表 4）：在 625 个探针中有 63 个反解未达 1e-7 的 bow 60 参考架上，恒等误差仍然是 **8.039e-14 px**，与直线参考架完全相同。

这正是"把 `p − hv_child(ℓ)` 留在公式里"的全部理由。旧的纯弦式实现没有这一项，child 导引线一弯，整个图案就整体位移几十像素（实测振幅 10 时 28 px，振幅 60 时 157 px），哪怕两块板画得一模一样。
...
前两行是 §8 描述的有限步收敛；后两行是阻尼的效果——**卡在平台上，而不是像朴素牛顿那样在 6.6/5.4/15.9 之间永久振荡**。停在平台上的点，其误差被残差项完整承接。
...
```mermaid
flowchart TD
    F["牛顿未收敛"] --> R1["① solve 返回历史最优迭代<br/>下界 = 弦基种子 = 旧实现的答案"]
    R1 --> R2["② 残差项 p − hv_child(ℓ) 吸收剩余误差<br/>映射保持连续，恒等性不受影响"]
    R2 --> R3["③ 朝向判定用 directions 的平滑切线<br/>与 solve 的精度解耦，折叠分类不受污染"]
    R3 --> OKK["输出仍然可用"]

    style OKK fill:#d5e8d4,stroke:#82b366
```

三层含义分别对应代码里的三处注释：`solve` 的 docstring 末段、`build_mapper` 的 docstring "The residual … stays in the formula as the fallback"、`directions` 的 docstring 首段。
