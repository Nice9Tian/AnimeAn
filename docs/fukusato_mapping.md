...
若某个顶点被闭合切缝与全部手柄完全隔离，所有测地权重均为零；实现对该顶点退回未乘手柄测度的欧氏权重，避免整个孤立片冻结。这是稳定性保护，不是论文公式的一部分。
...
**描边。** 系统为所有变形 UV 三角形包围盒建立均匀空间索引。每个纹理线段只与候选三角形做半平面参数裁剪；裁剪片段经 $A_\tau$ 映射后，仅在以下条件同时满足时串接：源参数连续、三角形相同或共享非切缝边、UV 三角形行列式符号相同。复制顶点使折角两侧不再邻接，因此跨折角线的纹理会真正断开。共享边上的重复片段只有在两侧仿射结果一致时才去重；重叠 UV 片的不同图像均保留。
...
该片标记为 `fukusato_mapped_back`，与正向片分层保存；算法检测但不自动修复翻转。描边宽度使用全局近似比例
...
4. 拖动只平移覆盖层的 `after` 曲线，不求解 UV，也不修改输出纹理。
5. 点击勾后，把手柄标记为已接受，从基准网格重新求解该帧全部已接受手柄，并执行矢量回写。
6. 新输出层完整生成后才删除旧的 Fukusato 输出层；任一步失败都会丢弃新层并保留旧结果。
7. 点击叉会删除新手柄；若正在修改一条已接受手柄，则恢复其修改前位置。删除已接受手柄时，系统用剩余手柄重新求解。
...
`tests/test_fukusato_mapping.py` 包含 12 个测试，覆盖：
...
- 当前按单个服装闭合区域求解，不处理不同裁片之间的缝线纹理连续性；这也是原论文列出的未来工作。
